% utils.ParforProgressbar   Progress monitor for `parfor` loops
%    ppm = utils.ParforProgressbar(numIterations) constructs a utils.ParforProgressbar object.
%    'numIterations' is an integer with the total number of
%    iterations in the parfor loop.
%
%    ppm = utils.ParforProgressbar(___, 'showWorkerProgress', true) will display
%    the progress of all workers (default: false).
%    
%    ppm = utils.ParforProgressbar(___, 'progressBarUpdatePeriod', 1.5) will
%    update the progressbar every 1.5 second (default: 1.0 seconds).
%
%    ppm = utils.ParforProgressbar(___, 'title', 'my fancy title') will
%    show 'my fancy title' on the progressbar.
%
%    ppm = utils.ParforProgressbar(___, 'parpool', 'local') will
%    start the parallel pool (parpool) using the 'local' profile.
%
%    ppm = utils.ParforProgressbar(___, 'parpool', {profilename, poolsize, Name, Value}) 
...
%    ppm = utils.ParforProgressbar(numIterations);
%
...
%          import libUtil.utils.ParforProgressbar;
         % loadobj - METHOD Reconstruct a utils.ParforProgressbar object
         
         % Once we've been loaded, we need to reconstruct ourselves correctly as a
         % worker-side object.
         debug('LoadObj');
         o = utils.ParforProgressbar( {X.ServerName, X.ServerPort, X.totalIterations, X.numWorkersPossible, X.stepSize, X.UserData} );
      end
   end
...
%            import libUtil.progressbar;
           % utils.ParforProgressbar - CONSTRUCTOR Create a utils.ParforProgressbar object
           % 
           %    ppb = utils.ParforProgressbar(numIterations)
           %    numIterations is an integer with the total number of
           %    iterations in the parfor loop.
           %
           %    ppm = utils.ParforProgressbar(___, 'showWorkerProgress', true) will display
           ...
           if iscell(numIterations) % worker
               ...
               if o.showWorkerProgress
                   ...
                   utils.progressbar(titles{:});
               else
                   if ~any(contains(p.UsingDefaults,'title'))
                       utils.progressbar(p.Results.title);
                   else
                       utils.progressbar;
                   end
               end
...
%     import libUtil.progressbar;
    ...
    if progressTotal > o.progressTotalOld
        ...
        if(o.showWorkerProgress)
            ...
            utils.progressbar(progressTotal, progWorkerC{:});
        else
            utils.progressbar(progressTotal);
        end
    end
end
