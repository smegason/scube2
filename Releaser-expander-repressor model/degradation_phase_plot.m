...
% in the degradation constant and the release rate
...
num_iterations = numel(Phi_HSs) * numel(c_HSs);
data = zeros(1, num_iterations);

% for parfor loop later on
coords = zeros(numel(Phi_HSs), numel(c_HSs), 2);
...
data = reshape(data, numel(Phi_HSs), numel(c_HSs));
...
h = imagesc([1/100,100], [1/100,100], data);
xlabel('Relative change in degradation rate c_{HS} / c_H');   
